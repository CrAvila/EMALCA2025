...
CleanData <-  read.csv("./HarmonyHealthcareData.csv", stringsAsFactors = FALSE)
...
regfit.full <- regsubsets(Admission ~ Patient.HCC.Risk.Total.Risk + Active.Medications +
    Primary.Care.Encounter.Count + SDOH.Assessment.Count +
    Patient.Appointment.No.Show.Rate.. + Depression.Screening.Count.Past.Yr +
    eGFR.Result + Most.Recent.BMI.Value + UDS.Qualifying.Encounter.Count +
    COVID.19.Immunization.Code + Fasting.Glucose.Test.Result,
...
```{r}
k <- 10
n <- nrow(new_train_data)
set.seed(1)
folds <- sample(rep(1:k, length = n))
cv.errors <- matrix(NA, k, 137,
    dimnames = list(NULL, paste(1:137)))
```


```{r}
for (j in 1:k) {
  best.fit <- regsubsets(Admission ~ Patient.HCC.Risk.Total.Risk + Active.Medications +
    Primary.Care.Encounter.Count + SDOH.Assessment.Count +
    Patient.Appointment.No.Show.Rate.. + Depression.Screening.Count.Past.Yr +
    eGFR.Result + Most.Recent.BMI.Value + UDS.Qualifying.Encounter.Count +
    COVID.19.Immunization.Code + Fasting.Glucose.Test.Result,
       data = new_train_data[folds != j, ],
       nvmax = 137)
  for (i in 1:137) {
    pred <- predict(best.fit, new_train_data[folds != j, ], id = i)
    cv.errors[j, i] <-
         mean((new_train_data$Admission ~ Patient.HCC.Risk.Total.Risk + Active.Medications +
    Primary.Care.Encounter.Count + SDOH.Assessment.Count +
    Patient.Appointment.No.Show.Rate.. + Depression.Screening.Count.Past.Yr +
    eGFR.Result + Most.Recent.BMI.Value + UDS.Qualifying.Encounter.Count +
    COVID.19.Immunization.Code + Fasting.Glucose.Test.Result[folds == j] - pred)^2)
   }
 }
```
...
```{r}
full_pred_raw <- predict(regfit.bwd, newdata = new_test_data)
